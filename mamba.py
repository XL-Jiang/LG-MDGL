...
class MambaConfig:
    ...
    expand_factor: int = 1 # E in paper/comments #膨胀因子
    d_conv: int = 4 #卷积核大小
    ...
    pscan: bool = True # use parallel scan mode or sequential mode when training 是否并行扫描
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        xz = self.in_proj(x) # (B, L, 2*ED) #输入投影到双倍维度r
        x, z = xz.chunk(2, dim=-1) # (B, L, ED), (B, L, ED) #按顺序分解为两条分支
        #x用于SSM，z用于门控
        # x branch
        x = x.transpose(1, 2) # (B, ED, L)
        x = self.conv1d(x)[:, :, :L] # depthwise convolution over time, with a short filter #截断处理确保输入与输出的序列长度相同
